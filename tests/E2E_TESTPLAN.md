...
1. 앱을 실행해 Process, Profile(편집/복사/삭제/정렬), ModKeys 세트 콤보, Start, Quick Events, Settings를 확인하고, 프로필 편집을 열어 Graph 버튼을 확인한다.
...
1. 조건 평가 전용 이벤트를 만든다.
2. 화면 없이 조건만 있는 입력 이벤트를 저장하고, 조건이 맞을 때만 키가 나가는지 확인한다.
3. 실행 이벤트 두 개를 같은 그룹에 서로 다른 priority로 구성하고 하나에 조건을 연결한다.
4. Graph에서 조건 방향과 그룹 관계를 확인한다.
5. 중복 이름, 필수 키 누락, 순환 조건을 저장하려 할 때 차단되는지 확인한다.
