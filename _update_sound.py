...
def update_sound_variable(sound_type: str, new_sound_path: str):
    ...
    if not new_sound_path.lower().endswith(supported_extensions):
        print(f"Error: Unsupported file type. Please use one of {supported_extensions}.")
        return
    ...
    try:
        ...
        if not pattern.search(content):
            print(f"Error: Could not find the '{variable_name}' variable in '{target_file}'.")
            return
...
def main():
    ...
    if len(sys.argv) != 3:
        ...
        print("Example: python _update_sound.py start \"C:\\Users\\Me\\sounds\\new_start.mp3\"")
        sys.exit(1)
