...
3.  캡처된 내용은 자동으로 `Quick.json` 프로필에 저장됩니다. 나중에 정식 에디터에서 불러와서 다듬으면 편합니다.
...
## 10. 리팩토링 검증용 사용자 테스트 시나리오

이 섹션은 **대규모 구조 리팩토링 이후에도 실제 사용자 흐름이 유지되는지** 확인하기 위한 수동 테스트 체크리스트입니다.
...
*   `Quick` 프로필 유지
*   테스트용 새 프로필 1개 (`RefactorTest` 권장)
*   화면에서 색상 변화가 확실한 영역 1개 (예: 브라우저 탭, 이미지 뷰어, 고정 UI)

기록 방법:

*   각 시나리오마다 `PASS / FAIL / N/A`
*   실패 시 **단계 번호, 실제 결과, 스크린샷/로그 경로** 기록

---

### 시나리오 A — 앱 시작 / 기본 화면 확인
...
*   실행 직후 예외 팝업 없이 창이 열린다
*   종료 후 재실행해도 동일하게 열린다
*   프로필 목록과 주요 버튼이 정상 표시된다

---

### 시나리오 B — 프로필 생성 / 복사 / 삭제

목적: 프로필 관리와 canonical name 기반 동작이 유지되는지 확인
...
*   복사본이 생성된다
*   삭제 후 깨진 선택 상태가 남지 않는다
*   드롭다운 표시 이름(즐겨찾기 포함)이 내부 동작을 망치지 않는다

---

### 시나리오 C — Quick Events 저장 흐름

목적: 빠른 이벤트 생성 기능이 여전히 작동하는지 확인
...
*   좌표와 이미지 캡처가 동작한다
*   저장 피드백이 즉시 갱신된다
*   `Quick` 프로필에서 실제 이벤트로 확인된다

---

### 시나리오 D — 프로필 편집기 기본 생성 / 수정 / 자동 저장

목적: 가장 중요한 편집 흐름이 리팩토링 후에도 유지되는지 확인
...
*   이벤트 추가/수정이 가능하다
*   편집 후 목록이 즉시 갱신된다
*   별도 저장 버튼 없이 자동 저장 결과가 유지된다

---

### 시나리오 E — 이벤트 검증 규칙

목적: 편집기 검증 규칙이 그대로 살아있는지 확인
...
*   조건/그룹 설정이 저장된다
*   Graph 창이 열리고 이벤트 관계가 보인다
*   누락 참조나 순환이 있으면 시각/문구로 확인 가능하다

---

### 시나리오 G — Settings / ModKeys / 상태 복원

목적: 설정 창과 보조 창 흐름, 상태 저장이 유지되는지 확인
...
6. 창 위치를 옮긴 뒤 닫고 다시 열어 위치 복원이 대체로 자연스러운지 확인

기대 결과:

*   설정 변경이 유지된다
*   ModKeys 설정이 저장된다
*   설정 창 재오픈 시 비정상 위치/초기화 문제가 없다

---

### 시나리오 H — Start / Stop 기본 실행

목적: 리팩토링 후 메인 실행 흐름이 깨지지 않았는지 확인
...
*   Start 직후 앱이 멈추지 않는다
*   Stop 시 정상 종료된다
*   반복 실행해도 중복 스레드/중복 시작 느낌이 없다

---

### 시나리오 I — 실제 화면 매칭 1건 스모크 테스트

목적: 최소 1개의 실제 자동화 이벤트가 동작하는지 확인

1. 픽셀 매칭 또는 작은 영역 매칭 이벤트 1개 준비
2. 쉽게 통제 가능한 화면 변화(예: 밝은/어두운 버튼, 탭 색상 변화)를 준비
3. `Start` 실행
...
*   인증 UI가 열린다
*   성공 시 메인 앱 전환
*   실패 시 오류 및 countdown이 중복 없이 한 번만 시작된다

---

### 리팩토링 회귀 체크 포인트

아래 항목은 이번 구조 재편에서 특히 중요합니다.
