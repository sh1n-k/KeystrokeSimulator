...
- 빌드 래퍼(macOS): `./build_secure.command`
- 빌드 래퍼(Windows): `build_secure.cmd`

## Current Structure
- `main.py`: 표준 GUI 진입점
- `main_secure.py`: 인증 포함 GUI 진입점
- `scripts/build_secure.py`: secure 배포용 PyInstaller 빌드 스크립트
- `build_secure.command`: macOS용 secure 빌드 래퍼
- `build_secure.cmd`: Windows용 secure 빌드 래퍼
...
- 제거된 레거시 루트 모듈명(`keystroke_*`, `i18n`, `profile_display`, `runtime_toggle_*`)은 다시 만들거나 import하지 않는다.
...
## Build / Auth Notes
- `scripts/build_secure.py`는 `main_secure.py`의 `os.getenv(...)`를 현재 환경값으로 치환한 뒤 PyInstaller를 실행한다.
